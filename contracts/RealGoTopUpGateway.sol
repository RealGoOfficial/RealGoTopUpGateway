...
// File contracts/RealGoTopUpGateway.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.20;
contract RealGoTopUpGateway is AccessControl, ReentrancyGuard {
    ...
    event WhitelistUpdated(address token, bool allowed);
    event PausedChanged(bool paused);
    event EmergencyStopped();
    event RescueToken(address token, uint256 amount, uint256 timestamp);
    event RescueNative(uint256 amount, uint256 timestamp);

    modifier whenNotPaused() {
        require(!paused && !emergencyStopped, "paused/emergency");
        _;
    }

    constructor(address _treasury, address[] memory initialWhitelist) {
        ...
        for (uint i = 0; i < initialWhitelist.length; i++) {
            tokenWhitelist[initialWhitelist[i]] = true;
    ...
    function setPaused(bool v) external onlyRole(ADMIN_ROLE) {
        paused = v;
    ...
    function setWhitelist(address token, bool allowed) external onlyRole(ADMIN_ROLE) {
    ...
    // ------------------------
    // Native currency top-up
    // ------------------------

    function depositNative(string calldata userId, string calldata orderId)
        external
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        require(bytes(userId).length > 0 && bytes(orderId).length > 0, "invalid id");

        Address.sendValue(payable(treasury), msg.value);

        emit DepositNative(msg.sender, block.timestamp, msg.value, userId, orderId);
    }
    ...
    function depositToken(address token, uint256 amount, string calldata userId, string calldata orderId)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        emit DepositToken(msg.sender, block.timestamp, token, amount, userId, orderId);
    }
    ...
    function rescueToken(address token, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit RescueToken(token, amount, block.timestamp);
    }

    /// @notice Rescue native ETH/BNB mistakenly sent to this contract
    function rescueNative(uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        (bool success, ) = payable(treasury).call{value: amount}("");
        require(success, "Native transfer failed");
        emit RescueNative(amount, block.timestamp);
    }
